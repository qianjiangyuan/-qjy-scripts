  ...
  <div>
    <template  v-if="showDetail==false">
      <el-form ref="form" :inline="true" class="demo-form-inline">
        ...
        <div>
          <div class="inptH32 mg-b10 pd-b5 border-b1">
            <el-form-item style="">
              <el-button size="small" type="primary" @click="addDlservice">新增</el-button>
            </el-form-item>
            <el-form-item style="">
              <el-button size="small"  @click="deleteDlservice" :disabled = hideDelete>删除</el-button>
            </el-form-item>
          </div>
          <div>
      ...
      <template v-if="showDetail">
        <dlservice-form :data="item"   @oncancel="cancelAddOrUpdateModel" @onsuccess="commitAddOrUpdateModel"  :readOnly="readOnly" ></dlservice-form>
      </template>
   </div>
</template>

<script>
import { QueryDlserviceList, DeleteDlserviceList } from '@/api/platform.dlservice'
import DlserviceForm from './DlserviceForm'
export default {
  name: 'DlserviceList',
  components: {
    DlserviceForm
  ...
  methods: {
    queryList () {
      ...
      QueryDlserviceList(queryparams).then(res => {
        this.loading = false
        that.dataList = res
      }).catch(res => {
        this.loading = false
    ...
    deleteDlservice () {
      ...
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        DeleteDlserviceList(ids).then(res => {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
